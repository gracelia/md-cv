...
## 工作经历
`2022.6-23.6`
__数字大脑研究院，北京__，前端研发（因公司资金链断裂离职）
...
### Eva（AIGA产品）
`数研院 05/23`
是一款<strong>浏览器插件</strong>，底层基于自研的大模型，可以理解自然语言、拆解问题并分步生成互联网可执行的动作，能力包括购物、订票、外卖、预约行程等可通过互联网完成的事件。

`技术栈`
React，Plasmo

`职责`
1) 负责<strong>技术选型</strong>、<strong>MVP</strong>产品实现。负责算法与工程团队沟通协调
...
### AI中台
`数研院 11/22`
是一套用来<strong>构建大规模智能服务</strong>的基础设施，意在构建人工智能模型全生命周期管理和服务体系。前期帮助内部算法工程师提升模型训练效率和资源利用率，帮助公司降低资源浪费。后期对外开放私有化部署，帮助服务企业快速构建智能服务的基础设施。
...
### 前端脚手架、组件库
`数研院 06/22`
根据公司自身业务特点打造前端脚手架、前端组件库。保证团队内技术选型与规范的统一，保证产品风格一致，降低交互产研<strong>沟通成本</strong>，提升工程<strong>研发效率</strong>，保证项目<strong>高可维护性</strong>。
...
`职责`
1) 担任<strong>全栈开发</strong>的角色，独立负责标注平台和开放平台的交互设计、需求沟通、研发实现。功能包括标注工具、ocr数据管理、标注验证和模型反馈等。
   
2) <strong>复杂组件</strong>开发。开发包括图片缩放、旋转、矩形/多边形标注框绘制等功能的标注工具。抽象简单灵活的接口，可被<strong>第三方项目依赖</strong>和使用。
...
### 先知平台
`第四范式`
由范式最早的机器学习领域<strong>低代码</strong>产品发展而来，后随企业发展演变成以数据、算法、生产三大块为核心的AI应用开发平台。核心模块包括：数据中心，建模IDE，应用IDE，模型管理，应用管理，权限管理等。前端工程上，主要模块之间相互独立，同时依赖多个公共子项目，不同模块间业务上存在上下游依赖。总工程项目多达二十余个，是 <strong>大型复杂的前端工程项目</strong> 。
...
`职责`
1) 支持前端需求<strong>快速落地</strong>。开发包括图片滑动组合、裸眼3d和3d模型等功能。
...
### 智能信贷风控系统
`第四范式`
<strong>金融信贷</strong>和风控系统主要包括信贷审核及风控规则的配置，后端结合算法模型，制定千人千面的规则策略。模块包括决策引擎，审批，评分卡，黑白名单，数据报表，系统/权限管理等。
...
2) 通用业务组件研发，复杂组件研发，包括<strong>决策树组件</strong>等。
